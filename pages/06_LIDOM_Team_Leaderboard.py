...
def _metric_rank(df, stat, team="Leones del Escogido", ascending=False):
    r, v = rank_position(df, stat, team=team, ascending=ascending)
    return r, v, safe_fmt(stat, v)


def _summary_pack(title, hallazgos, oportunidades, recomendacion):
    return {
        "title": title,
        "hallazgos": [str(x).strip() for x in hallazgos if str(x).strip()][:3],
        "oportunidades": [str(x).strip() for x in oportunidades if str(x).strip()][:3],
        "recomendacion": str(recomendacion).strip(),
    }


def make_escogido_summary(section, hitting, baserunning):
    """GM-style summaries for Escogido using Hallazgos / Oportunidades / Recomendación."""
    team = "Leones del Escogido"

    if section == "hitting":
        ops_r, _, ops_s = _metric_rank(hitting, "OPS", team, ascending=False)
        obp_r, _, obp_s = _metric_rank(hitting, "OBP", team, ascending=False)
        slg_r, _, slg_s = _metric_rank(hitting, "SLG", team, ascending=False)
        bb_r, _, bb_s = _metric_rank(hitting, "BB%", team, ascending=False)
        hits_r, _, hits_s = _metric_rank(hitting, "Hits", team, ascending=False)
        hr_r, _, hr_s = _metric_rank(hitting, "Homerun", team, ascending=False)

        hallazgos = [
            f"La ofensiva tiene una identidad valiosa: disciplina, tráfico y turnos competitivos. OBP #{obp_r or '-'} ({obp_s}) y BB% #{bb_r or '-'} ({bb_s}) obligan al rival a trabajar bajo presión.",
            f"El volumen de contacto sostiene la producción diaria. Hits #{hits_r or '-'} ({hits_s}) confirma que el club puede crear oportunidades incluso sin depender del batazo largo.",
            "El enfoque de la ofensiva ya tiene una base ganadora: llegar a base, extender innings y forzar decisiones defensivas."
        ]
        oportunidades = [
            f"El próximo salto es transformar tráfico en daño. OPS #{ops_r or '-'} ({ops_s}) y SLG #{slg_r or '-'} ({slg_s}) muestran margen para impactar más con corredores en base.",
            f"El poder situacional debe pesar más. HR #{hr_r or '-'} ({hr_s}) limita la capacidad de cambiar juegos con un swing.",
            "La prioridad no es abandonar la disciplina; es combinarla con agresividad selectiva en conteos favorables."
        ]
        recomendacion = (
            "Mantener la ventaja de disciplina, pero orientar el plan ofensivo hacia más contacto de impacto. "
            "El objetivo debe ser convertir el tráfico existente en carreras reales sin sacrificar la identidad de zona."
        )
        return _summary_pack("Escogido Hitting Summary", hallazgos, oportunidades, recomendacion)

    if section == "baserunning":
        sb_r, _, sb_s = _metric_rank(baserunning, "SB", team, ascending=False)
        cs_r, _, cs_s = _metric_rank(baserunning, "CS", team, ascending=True)
        sbp_r, _, sbp_s = _metric_rank(baserunning, "SB%", team, ascending=False)
        sba_r, _, sba_s = _metric_rank(baserunning, "SBA", team, ascending=False)

        hallazgos = [
            f"El corrido de bases es parte real de la identidad del club. SBA #{sba_r or '-'} ({sba_s}) y SB #{sb_r or '-'} ({sb_s}) muestran intención de presionar cada inning.",
            "La agresividad obliga a pitchers, catchers e infielders a ejecutar perfecto, creando valor que no siempre aparece en la línea tradicional.",
            "El equipo tiene atletas y mentalidad para cambiar el ritmo del juego con las piernas."
        ]
        oportunidades = [
            f"La agresividad solo paga si gana el intercambio. CS #{cs_r or '-'} ({cs_s}) y SB% #{sbp_r or '-'} ({sbp_s}) deben guiar mejores decisiones.",
            "El ajuste no es correr menos; es correr con mejor información: pitcher, catcher, conteo, score e inning.",
            "Reducir outs evitables en bases puede tener impacto inmediato en producción de carreras."
        ]
        recomendacion = (
            "Sostener la presión, pero elevar el estándar de selección. Cada intento debe tener una razón táctica clara y una ventaja real."
        )
        return _summary_pack("Escogido Baserunning Summary", hallazgos, oportunidades, recomendacion)
    ...
    hallazgos = [
        f"Las tendencias muestran la forma del equipo, no solo el resultado final. OBP #{obp_r or '-'} ({obp_s}) y BB% #{bb_r or '-'} ({bb_s}) explican la base del rendimiento.",
        f"La presión en bases sigue presente en el perfil del club. SB #{sb_r or '-'} ({sb_s}) mantiene al rival defendiendo más de una amenaza.",
        "Cuando las líneas se estabilizan, el mensaje para el dugout es simple: repetir los procesos que sostienen tráfico y presión."
    ]
    oportunidades = [
        f"La señal a vigilar es la distancia entre crear tráfico y convertirlo en producción. OPS #{ops_r or '-'} ({ops_s}) debe acercarse al nivel del OBP.",
        f"En bases, SB% #{sbp_r or '-'} ({sbp_s}) dirá si el volumen realmente está agregando valor.",
        "Cualquier caída sostenida debe detectarse temprano, antes de que se convierta en tendencia de equipo."
    ]
    recomendacion = (
        "Usar las tendencias como herramienta de decisión diaria: si el proceso se mantiene pero el resultado cae, ajustar swing decisions y selección de robos antes de cambiar la identidad."
    )
    return _summary_pack("Escogido Team Rolling Summary", hallazgos, oportunidades, recomendacion)


def make_escogido_pitching_summary(pitching, label="Pitching"):
    if pitching is None or pitching.empty:
        return _summary_pack(
            f"Escogido {label} Summary",
            ["Carga el archivo de pitcheo para generar hallazgos."],
            ["No hay datos suficientes para evaluar oportunidades."],
            "Completar la carga de datos antes de tomar decisiones."
        )

    team = "Leones del Escogido"
    era_r, _, era_s = _metric_rank(pitching, "ERA", team, ascending=True)
    fip_r, _, fip_s = _metric_rank(pitching, "FIP", team, ascending=True)
    whip_r, _, whip_s = _metric_rank(pitching, "WHIP", team, ascending=True)
    k_r, _, k_s = _metric_rank(pitching, "K%", team, ascending=False)
    bb_r, _, bb_s = _metric_rank(pitching, "BB%", team, ascending=True)
    hr_r, _, hr_s = _metric_rank(pitching, "HR%", team, ascending=True)
    baa_r, _, baa_s = _metric_rank(pitching, "BAA", team, ascending=True)

    lower_label = str(label).lower()
    if "relief" in lower_label or "rp" in lower_label or "bullpen" in lower_label:
        hallazgos = [
            f"El bullpen debe evaluarse por la calidad de sus outs en momentos de presión. BB% #{bb_r or '-'} ({bb_s}) muestra cuánto control tiene el grupo sobre el inning.",
            f"FIP #{fip_r or '-'} ({fip_s}) y WHIP #{whip_r or '-'} ({whip_s}) explican si el relevo está entrando y saliendo limpio de tráfico.",
            "El valor del relevo no es solo prevenir carreras; es proteger ventajas, cortar rallies y terminar entradas."
        ]
        oportunidades = [
            f"El techo del bullpen sube cuando puede terminar turnos por dominio. K% #{k_r or '-'} ({k_s}) es una de las señales clave.",
            f"HR% #{hr_r or '-'} ({hr_s}) y ERA #{era_r or '-'} ({era_s}) deben leerse juntos: un swing puede cambiar la noche completa.",
            "El foco debe estar en leverage: atacar temprano, evitar boletos y obligar contacto débil cuando el juego está apretado."
        ]
        rec = (
            "Definir roles alrededor de confiabilidad en zona y ejecución bajo presión. En el bullpen, el mejor indicador no es solo stuff: es quién puede convertir ventaja en final."
        )
        return _summary_pack(f"Escogido {label} Summary", hallazgos, oportunidades, rec)

    hallazgos = [
        f"La rotación debe evaluarse más allá del ERA. FIP #{fip_r or '-'} ({fip_s}) y HR% #{hr_r or '-'} ({hr_s}) muestran qué tan bien controla los factores que más dañan.",
        f"WHIP #{whip_r or '-'} ({whip_s}) y BAA #{baa_r or '-'} ({baa_s}) cuentan la historia del tráfico: cuántos innings se vuelven difíciles antes del batazo grande.",
        "El objetivo de los abridores es mantener juegos ganables y entregar innings con estructura."
    ]
    oportunidades = [
        f"El techo del grupo depende de generar más outs por dominio. K% #{k_r or '-'} ({k_s}) define cuánto margen tiene el staff con hombres en base.",
        f"BB% #{bb_r or '-'} ({bb_s}) debe sostenerse o mejorar para que el contacto permitido no se convierta en rallies largos.",
        f"ERA #{era_r or '-'} ({era_s}) debe acercarse al perfil subyacente si la ejecución en conteos favorables mejora."
    ]
    rec = (
        "Priorizar planes que aumenten swing-and-miss sin regalar bases. La rotación no necesita reinventarse; necesita cerrar turnos con más autoridad."
    )
    return _summary_pack(f"Escogido {label} Summary", hallazgos, oportunidades, rec)
...
def make_escogido_defense_summary(defense):
    if defense is None or defense.empty:
        return _summary_pack(
            ...
            ["Carga los archivos de defensa para generar hallazgos."],
            ["No hay datos suficientes para evaluar oportunidades."],
            "Completar la carga de datos antes de tomar decisiones."
        )
    team = "Leones del Escogido"
    cs_r, _, cs_s = _metric_rank(defense, "CS%", team, ascending=False)
    ife_r, _, ife_s = _metric_rank(defense, "IFErr", team, ascending=True)
    iff_r, _, iff_s = _metric_rank(defense, "IFFld%", team, ascending=False)
    ofe_r, _, ofe_s = _metric_rank(defense, "OFErr", team, ascending=True)
    off_r, _, off_s = _metric_rank(defense, "OFFld%", team, ascending=False)

    hallazgos = [
        f"El infield marca el estándar defensivo del club. IFErr #{ife_r or '-'} ({ife_s}) e IFFld% #{iff_r or '-'} ({iff_s}) reducen innings extendidos.",
        f"CS% #{cs_r or '-'} ({cs_s}) aporta contexto sobre cuánto el catching ayuda al pitcheo a controlar el juego de correr.",
        "La defensa es parte directa de la prevención de carreras: cada out rutinario convertido le quita aire al rival."
    ]
    oportunidades = [
        f"La lectura completa exige balancear infield y outfield. OFErr #{ofe_r or '-'} ({ofe_s}) y OFFld% #{off_r or '-'} ({off_s}) muestran dónde está el margen.",
        "El objetivo no es verse limpio; es quitarle al rival outs extra, bases gratis e innings extendidos.",
        "La comunicación en los jardines y la consistencia en jugadas rutinarias deben ser punto de énfasis."
    ]
    rec = (
        "Elevar el estándar del outfield al nivel del infield. Si la defensa completa reduce errores gratis, el pitcheo recibe un salto inmediato de apoyo."
    )
    return _summary_pack("Escogido Defense Summary", hallazgos, oportunidades, rec)


def _fit_lines_to_box(lines, max_width, max_height, start_size=6.5, min_size=5.0, leading_mult=1.22):
    for size in [start_size, 6.2, 5.9, 5.6, 5.3, min_size]:
        avg_char = max(size * 0.48, 2.6)
        max_chars = max(24, int(max_width / avg_char))
        wrapped = []
        for line in lines:
            wrapped.extend(textwrap.wrap(str(line), max_chars) or [""])
        leading = size * leading_mult
        if len(wrapped) * leading <= max_height:
...
def _draw_text_lines(c, lines, x, y, max_width, max_height, color="#111111", start_size=6.5):
    size, leading, wrapped = _fit_lines_to_box(lines, max_width, max_height, start_size=start_size)
    c.setFillColor(colors.HexColor(color))
...
def draw_summary_box(c, title, body, x, y, w, h, logo_paths):
    """Executive summary with Hallazgos, Oportunidades and Recomendación."""
    theme = "#BA0C2F"
    c.setStrokeColor(colors.HexColor(theme))
    c.setLineWidth(0.9)
    c.setFillColor(colors.white)
    c.roundRect(x, y, w, h, 7, fill=1, stroke=1)

    safe_draw_image(c, logo_paths.get("escogido"), x + 10, y + h - 25, 34, 18)

    title_text = body.get("title", title) if isinstance(body, dict) else title
    c.setFillColor(colors.HexColor(theme))
    c.setFont("Helvetica-Bold", 8.0)
    c.drawString(x + 52, y + h - 18, str(title_text).upper())

    if not isinstance(body, dict):
        _draw_text_lines(c, str(body).split("\n"), x + 12, y + h - 38, w - 24, h - 46, start_size=6.1)
        return
    ...
    def mini_box(mx, title2, bullets, check=True):
        c.setStrokeColor(colors.HexColor(theme))
        c.setLineWidth(0.55)
        c.roundRect(mx, box_y, col_w, col_h, 4, fill=0, stroke=1)
        heading_y = box_y + col_h - 10
        c.setFillColor(colors.HexColor(theme))
        c.setFont("Helvetica-Bold", 6.6)
        heading = title2.upper()
        c.drawString(mx + 7, heading_y, heading)
        c.line(mx + 7, heading_y - 1.8, mx + 7 + c.stringWidth(heading, "Helvetica-Bold", 6.6), heading_y - 1.8)
        prefix = "✓ " if check else "• "
        lines = [prefix + b for b in bullets]
        _draw_text_lines(c, lines, mx + 7, heading_y - 10, col_w - 14, col_h - 15, start_size=5.75)

    mini_box(left_x, "Hallazgos", body.get("hallazgos", []), check=True)
    mini_box(right_x, "Oportunidades", body.get("oportunidades", []), check=False)

    rec_y = y + 8
    c.setFillColor(colors.HexColor("#FFF7F8"))
    c.setStrokeColor(colors.HexColor(theme))
    c.roundRect(x + 12, rec_y, w - 24, rec_h, 4, fill=1, stroke=1)
    c.setFillColor(colors.HexColor(theme))
    c.setFont("Helvetica-Bold", 6.2)
    c.drawString(x + 20, rec_y + rec_h - 8, "RECOMENDACIÓN EJECUTIVA")
    _draw_text_lines(c, [body.get("recomendacion", "")], x + 20, rec_y + 7, w - 40, 10, start_size=5.4)
...
def to_pdf(hitting: pd.DataFrame, baserunning: pd.DataFrame, rolling_hitting: pd.DataFrame, rolling_baserunning: pd.DataFrame, pitching: pd.DataFrame | None = None, defense: pd.DataFrame | None = None, logo_uploads: dict | None = None) -> BytesIO:
    ...
    # Full-width executive summary so GM-style text does not get clipped
    draw_summary_box(
        c,
    ...
    c.roundRect(54, 178, W - 108, 44, 8, fill=1, stroke=1)
    safe_draw_image(c, logo_paths.get("escogido"), 66, 188, 54, 25)
    c.setFillColor(colors.HexColor("#A00012"))
    c.setFont("Helvetica-Bold", 10)
    c.drawString(132, 206, "ESCOGIDO BASERUNNING SNAPSHOT")
    c.setFont("Helvetica-Bold", 9)
    c.drawString(132, 190, f"SB: #{sb_r or '-'} ({format_value('SB', sb_v) if sb_v is not None else '-'})   |   CS: #{cs_r or '-'} ({format_value('CS', cs_v) if cs_v is not None else '-'})   |   SB%: #{sbp_r or '-'} ({format_value('SB%', sbp_v) if sbp_v is not None else '-'})")
